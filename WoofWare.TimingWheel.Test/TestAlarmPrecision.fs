...
module TestAlarmPrecision =
  ...
  ""00s.134217700 (134217728 ns)""
            ...
            return
                [ -3 .. 3 ]
                |> List.map (AlarmPrecision.div AlarmPrecision.aboutOneSecond >> AlarmPrecision.display)
        }
  ...
  ""08s.589934500 (8589934592 ns)""
            ...
            return
                [ -3 .. 3 ]
                |> List.map (AlarmPrecision.mul AlarmPrecision.aboutOneSecond >> AlarmPrecision.display)
        }
    ...
    let ``test ofSpanFloorPow2Ns`` () : unit =
        for t in
            ...
            if AlarmPrecision.toSpan t > TimeNs.Span.nanosecond then
                t
                |> shouldEqual (
                    (AlarmPrecision.toSpan t + TimeNs.Span.nanosecond)
                    |> AlarmPrecision.ofSpanFloorPow2Ns
                )
